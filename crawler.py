...
def get_nonogram_from_html(html):
    ...
    # Nonogram columns block
    nmtt_body = nonogram_table.find("td", { "class": "nmtt" }).find("table").find("tbody")

    # Count of all `td` in any `tr` of nmtt_body is a count of columns
    columns_count = len(nmtt_body.find("tr").findAll("td"))
    ...
    # Count of all `tr` of nmtl_body is a count of rows
    rows_count = len(nmtl_trs)
